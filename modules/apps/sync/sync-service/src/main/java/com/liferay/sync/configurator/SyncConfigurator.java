...
public class SyncConfigurator {
	...
	protected void activate(BundleContext bundleContext) {
		...
		for (Company company : companies) {
			...
			String lanServerId = PrefsPropsUtil.getString(
				company.getCompanyId(),
				SyncServiceConfigurationKeys.SYNC_LAN_SERVER_ID);

			if (lanEnabled && Validator.isNull(lanServerId)) {
				try {
					SyncUtil.updateLanKeyPair(company.getCompanyId());
				}
				catch (Exception e) {
					_log.error(e, e);
				}
